...
def _strip_markdown_fence(raw: str) -> str:
    """Unwrap exactly one whole-message Markdown code fence.

    Some providers wrap otherwise valid JSON in a single ```json fence even
    when instructed to return only JSON. The fence is transport wrapping, not
    content, so it is removed before the SAME strict parse; prose before or
    after the fence, nested fences, and non-JSON content still fail closed.
    """
...
def _parse_fidelity_decisions(raw: str, n: int) -> List[bool]:
    ...
    try:
        parsed = json.loads(
            _strip_markdown_fence(raw), object_pairs_hook=_reject_duplicate_json_keys
        )
    except (TypeError, ValueError, RecursionError):
        return []
    if not isinstance(parsed, list) or len(parsed) != n:
